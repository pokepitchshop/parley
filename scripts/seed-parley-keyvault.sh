#!/usr/bin/env bash
# Seed Key Vault secrets after platform apply. Values from env — never committed.
# Azure OpenAI is keyless (managed identity) — no openai-key secret. See docs/llm-provider.md.
set -euo pipefail

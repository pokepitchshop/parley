...
class VoiceTwiMLServiceTest {
	...
	void respondWhenLlmFailsReturnsSpokenFallback() throws Exception {
		...
		given(voiceReplyService.replyToUtterance(CALL_SID, "+15551234567", "What are your hours?"))
				.willReturn(AgentGuardrails.LLM_UNAVAILABLE.trim());
	...
	void respondWithSpeechAcknowledgesAndRedirectsToReply() throws Exception {
		...
		verify(voiceReplyService, never()).replyToUtterance(anyString(), anyString(), anyString());
	}
